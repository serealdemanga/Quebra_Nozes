...
export async function registerUserWithPrimaryPortfolioAndSession(env: Env, input: RegisterRecordInput): Promise<void> {
  await env.DB.batch([
    ...
    ).bind(input.userId, input.cpf, input.email, input.passwordHash, input.displayName || null),
    env.DB.prepare(
      `INSERT INTO portfolios (
        id, user_id, name, is_primary, created_at, updated_at
      ) VALUES (?, ?, 'Carteira Principal', 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`
    ).bind(input.portfolioId, input.userId),
    env.DB.prepare(
    ...
    ).bind(input.sessionId, input.userId, input.sessionTokenHash, input.deviceFingerprint || null, input.userAgent || null, input.ipAddress || null, input.rememberDevice ? 1 : 0, input.sessionExpiresAt)
  ]);
}
...
export async function createSession(env: Env, input: SessionInsertInput): Promise<void> {
  ...
  ).bind(input.sessionId, input.userId, input.sessionTokenHash, input.deviceFingerprint || null, input.userAgent || null, input.ipAddress || null, input.rememberDevice ? 1 : 0, input.sessionExpiresAt).run();
}
...
export async function findSessionStateByTokenHash(env: Env, tokenHash: string): Promise<SessionStateRow | null> {
  return await env.DB.prepare(
    `SELECT
     ...
     LIMIT 1`
  ).bind(tokenHash).first<SessionStateRow>();
}
